...
export default function Blog() {
  return (
    <section id="blog" className="snap-start py-32 relative overflow-hidden bg-[#0b0310]">
      {/* Background Decorative Element */}
      <div className="absolute bottom-[10%] right-[-5%] w-[400px] h-[400px] bg-rose-400/[0.02] blur-[120px] -z-10 rounded-full"></div>
      ...
      <div className="container mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
          {posts.map((post, index) => (
            <motion.article
              key={index}
              className="glass-card group flex flex-col h-full cursor-pointer hover:border-rose-400/20 transition-all duration-500 relative rounded-none"
            ...
            >
              ...
              <div className={`h-52 w-full ${post.image} relative overflow-hidden shrink-0`}>
                ...
                <div className="absolute top-4 left-4 bg-black/90 border border-white/10 text-rose-400 font-creative text-[9px] font-black tracking-widest px-4 py-1.5 rounded-none uppercase shadow-glow">
                  {post.category}
                </div>
              </div>

              {/* Content */}
              <div className="p-6 bg-[#140521] border-t border-white/5 flex-grow flex flex-col justify-between">
                <div>
                  <p className="font-creative text-[9px] font-black tracking-widest text-slate-500 uppercase mb-3">{post.date}</p>
                  <h3 className="text-xl font-display font-black text-white mb-3 group-hover:text-rose-400 transition-all duration-300 line-clamp-2 uppercase tracking-wide">
                    {post.title}
                  </h3>
                ...
                <div className="flex items-center text-rose-400 font-creative text-[10px] font-black uppercase tracking-widest group-hover:translate-x-2 transition-transform duration-500">
