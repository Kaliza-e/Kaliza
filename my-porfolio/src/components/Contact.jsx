...
export default function Contact() {
  return (
    <section id="contact" className="snap-start py-32 relative overflow-hidden bg-[#0b0310]">
      {/* Background Glows */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-rose-400/[0.015] blur-[150px] -z-10 rounded-full"></div>
        ...
        <div className="grid lg:grid-cols-5 gap-16 max-w-6xl mx-auto">
          ...
          >
            ...
            <div className="space-y-8">
              {[
                { icon: FiMail, label: 'Email', value: 'kalizaesther5@gmail.com', href: 'mailto:kalizaesther5@gmail.com', color: 'text-rose-400', bg: 'bg-rose-400/5' },
                { icon: FiPhone, label: 'Phone', value: '+250 728 045 049', href: 'tel:+250728045049', color: 'text-rose-300', bg: 'bg-rose-400/5' },
                { icon: FiMapPin, label: 'Location', value: 'Kigali, Rwanda', color: 'text-rose-400', bg: 'bg-rose-400/5' },
              ].map((item, idx) => (
                <div key={idx} className="flex items-start gap-6 group relative">
                  <div className={`w-12 h-12 ${item.bg} ${item.color} flex items-center justify-center flex-shrink-0 border border-white/10 group-hover:border-rose-400 transition-all duration-300 shadow-glow rounded-none`}>
                    <item.icon size={18} />
                  </div>
                  <div>
                    <h4 className="font-creative text-[9px] font-black tracking-[0.25em] uppercase text-slate-500 mb-1">{item.label}</h4>
                    {item.href ? (
                      <a href={item.href} className="font-creative uppercase tracking-wider text-white text-sm hover:text-rose-400 transition-all">{item.value}</a>
                    ) : (
            ...
            <form className="glass-card p-8 flex flex-col gap-6 rounded-none relative" onSubmit={(e) => e.preventDefault()}>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div className="flex flex-col gap-2.5">
                  ...
                  <input 
                    type="text" 
                    id="name" 
                    className="bg-black/50 border border-white/10 rounded-none px-5 py-3.5 text-white focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 transition-all placeholder:text-slate-700 font-creative text-xs uppercase tracking-wider"
              ...
              <div className="flex flex-col gap-2.5">
                ...
                <textarea 
                  id="message" 
                  rows="4" 
                  className="bg-black/50 border border-white/10 rounded-none px-5 py-3.5 text-white focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 transition-all placeholder:text-slate-700 font-creative text-xs uppercase tracking-wider resize-none"
